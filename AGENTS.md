...
7. **Sabah İstihbaratı 4 Kilit Madde Uzunluk Sınırı (Kompakt ve Hap Bilgi Prensibi):**
   - Sabah İstihbaratı 4 Kilit Madde kartlarının (`morningBrief.bullets`) her biri KESİNLİKLE EN FAZLA 2-3 vurucu ve akıcı cümleden oluşmalıdır (Maksimum 45-60 kelime / 400-500 karakter).
   - Kullanıcının dünyada olup biteni 30 saniyede yakalayabilmesi amacıyla metinler kompakt, jilet gibi net ve hap bilgi niteliğinde tutulur; sayfayı dikeyde gereksiz uzatan destansı çok paragraflı anlatımlar KESİNLİKLE YASAKTIR.

---

## 🐦 11. X (Twitter) AI Nabzı & Seçkin 100 Hesap Standartları

1. **Seçkin 100 Hesap Listesi & Bütçe Koruma Prensibi:**
   - X gündemi için internetteki genel ve gürültülü akış yerine, yapay zeka ekosisteminin en saygın, tutkulu, üretken, bağımsız ve çılgın 100 öncüsü, araştırmacısı, açık kaynak mimarı ve çekirdek geliştiricisi taranır:
     - **1-15 (Batch 1 - Çekirdek Mimarlar & Araştırmacılar):** 1. `@simonw` (Simon Willison), 2. `@swyx` (Shawn Wang), 3. `@_philschmid` (Philipp Schmid), 4. `@jeremyphoward` (Jeremy Howard), 5. `@awnihannun` (Awni Hannun), 6. `@ggerganov` (Georgi Gerganov), 7. `@chipro` (Chip Huyen), 8. `@hwchase17` (Harrison Chase), 9. `@jerryjliu0` (Jerry Liu), 10. `@HamelHusain` (Hamel Husain), 11. `@karpathy` (Andrej Karpathy), 12. `@fchollet` (François Chollet), 13. `@srush_nlp` (Sasha Rush), 14. `@tri_dao` (Tri Dao), 15. `@Tim_Dettmers` (Tim Dettmers).
     - **16-30 (Batch 2 - Pratik Deneyciler, Görsel/Ajan Öncüleri & Analistler):** 16. `@rasbt` (Sebastian Raschka), 17. `@lilianweng` (Lilian Weng), 18. `@eugeneyan` (Eugene Yan), 19. `@emollick` (Ethan Mollick), 20. `@swann_vance` (Swann Vance), 21. `@wattenberger` (Amelia Wattenberger), 22. `@minchoi` (Min Choi), 23. `@fofrAI` (fofr), 24. `@LinusEkenstam` (Linus Ekenstam), 25. `@bilawalsidhu` (Bilawal Sidhu), 26. `@arankomatsuzaki` (Aran Komatsuzaki), 27. `@natolambert` (Nathan Lambert), 28. `@RisingSayak` (Sayak Paul), 29. `@cwolferesearch` (Cameron R. Wolfe), 30. `@rainisto` (Roope Rainisto).
     - **31-45 (Batch 3 - Bağımsız Hacker'lar, Açık Kaynak Geliştiricileri & Ajan Mimarları):** 31. `@danielhanchen` (Daniel Han - Unsloth), 32. `@Teknium1` (Teknium - Nous Research / Hermes), 33. `@corbtt` (Corbett - Cline), 34. `@jxnlco` (Jason Liu - Instructor), 35. `@yoheinakajima` (Yohei Nakajima - BabyAGI), 36. `@maximelabonne` (Maxime Labonne - Mergekit & LLM Course), 37. `@abacaj` (Anton Bacaj - Yerel Çıkarım & Benchmark), 38. `@mckaywrigley` (McKay Wrigley - Chatbot UI & Ajan Akışları), 39. `@levelsio` (Pieter Levels - Bağımsız Girişimci/Hacker), 40. `@svpino` (Santiago - AI Mühendisi & Eğitmen), 41. `@omarsar0` (Elvis Saravia - DAIR.AI), 42. `@charliebholtz` (Charlie Holtz - Canlı AI Deneyleri & Multimodal), 43. `@nutlope` (Hassan El Mghari - Vercel AI & Açık Araçlar), 44. `@rohanpaul_ai` (Rohan Paul - Pratik Kod Analistleri), 45. `@Yampeleg` (Yam Peleg - Hızlı Çıkarım & Fine-Tuning).
     - **46-60 (Batch 4 - Uygulamalı Yapay Zeka, Çıkarım, Donanım & Sistem Mühendisleri):** 46. `@drjimfan` (Jim Fan - Embodied AI & Ajanlar), 47. `@bindureddy` (Bindu Reddy - Abacus AI / Açık Model Savunucusu), 48. `@amasad` (Amjad Masad - Replit), 49. `@reach_vb` (Vaibhav Srivastav - Hugging Face Ses/Whisper), 50. `@osanseviero` (Omar Sanseviero - Hugging Face Topluluk), 51. `@alexalbert__` (Alex Albert - Anthropic DevRel), 52. `@charles_irl` (Charles Frye - Modal Labs GPU/Altyapı), 53. `@altryne` (Zohar Arad - Browser Ajanları & Otomasyon), 54. `@mattshumer_` (Matt Shumer - Reflection / HyperWrite), 55. `@mervenoyann` (Merve Noyan - Hugging Face Vision/Multimodal), 56. `@_jasonwei` (Jason Wei - Düşünce Zinciri / CoT Mimarı), 57. `@ID_AA_Carmack` (John Carmack - Keen Technologies / Bağımsız AGI), 58. `@GrantSlatton` (Grant Slatton - Büyük Sistemler & Kodlama Ajanları), 59. `@victormustar` (Victor Mustar - Açık AI Arayüzleri), 60. `@dan_jeffries1` (Dan Jeffries - Merkeziyetsiz Altyapı & AI Gerçekliği).
     - **61-74 (Batch 5 - Donanım, Derleyiciler, Çekirdek Hacker'lar & Açık Kaynak Efsaneleri):** 61. `@geohot` (George Hotz - tinygrad & comma.ai), 62. `@lucidrains` (Phil Wang - Makale Başına 24 Saatte PyTorch Kodu), 63. `@comfyanonymous` (ComfyUI Yaratıcısı), 64. `@cocktailpeanut` (Pinokio Yaratıcısı), 65. `@cloneofsimo` (Simo Ryu - Difüzyon LoRA Mimarı), 66. `@ylecun` (Yann LeCun - Meta Baş Bilim İnsanı), 67. `@gdb` (Greg Brockman - OpenAI Kurucu Ortağı), 68. `@adcock_brett` (Brett Adcock - Figure AI), 69. `@cHHillee` (Woosuk Kwon - vLLM & PagedAttention), 70. `@dhh` (David Heinemeier Hansson - On-Prem AI / Bağımsız Donanım), 71. `@goodside` (Riley Goodside - Scale AI / Prompt Injection & Jailbreak), 72. `@steipete` (Peter Steinberger - PSPDFKit & Vibe Coding), 73. `@mitchellh` (Mitchell Hashimoto - Ghostty & Terminal AI), 74. `@abhishekkrthakur` (Abhishek Thakur - 4x Kaggle Grandmaster).
     - **75-87 (Batch 6 - Otonom Ajanlar, Browser Botları, Üretken UI & Model Araştırmacıları):** 75. `@borisdayma` (Boris Dayma - Craiyon / DALL-E mini), 76. `@transitive_bs` (Travis Fischer - Agentic AI & AI SDK), 77. `@nearcyan` (Near - Compute & Cluster Analizleri), 78. `@paraschopra` (Paras Chopra - Model Deneyleri & AI Felsefesi), 79. `@bshaby` (Shyamal Anadkat - Uygulamalı AI & Ajanlar), 80. `@karinanguyen_` (Karina Nguyen - Ajan Güvenliği & Evals), 81. `@shadcn` (shadcn - Generative UI & Bileşen Mimarı), 82. `@rauchg` (Guillermo Rauch - Vercel / AI SDK & Web AI), 83. `@OfficialLoganK` (Logan Kilpatrick - Google AI Studio), 84. `@cubiq` (Matteo Spinelli - ComfyUI IP-Adapter), 85. `@bentossell` (Ben Tossell - AI Ürün Avcısı), 86. `@MattWolfe9` (Matt Wolfe - FutureTools), 87. `@rowancheung` (Rowan Cheung - The Rundown AI).
     - **88-100 (Batch 7 - Frontier Vizyonerleri, Kurucular & Ekosistem Mimarları):** 88. `@huyingxi` (Yingxi Hu - Üretken Video Pipeline), 89. `@chavinlo` (Chavinlo - Alpaca-LoRA), 90. `@jachiam0` (Josh Achiam - RL & Ajan Zekası), 91. `@natfriedman` (Nat Friedman - AI Grant & Açık Kaynak), 92. `@danielgross` (Daniel Gross - SSI), 93. `@ilyasut` (Ilya Sutskever - SSI / Süper Zeka), 94. `@sama` (Sam Altman - OpenAI), 95. `@demishassabis` (Demis Hassabis - Google DeepMind), 96. `@AravSrinivas` (Aravind Srinivas - Perplexity), 97. `@alexandr_wang` (Alexandr Wang - Scale AI), 98. `@paulg` (Paul Graham - Y Combinator), 99. `@mreflow` (Xavier Amatriain - Sistemik Mimariler), 100. `@dair_ai` (DAIR.AI - Açık Araştırma Laboratuvarı).
   - Apify aktörüne (`xquik~x-tweet-scraper`) 7 batch halinde arama sorgusu iletilir, maksimum 600 tweet taranır ve filtrelenmiş en kaliteli 180 tweet LLM analizine aktarılır.
